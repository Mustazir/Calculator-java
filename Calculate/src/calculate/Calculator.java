...
public class Calculator extends javax.swing.JFrame { //frame work inharitance
    ...
    public Calculator() { //for clock
        initComponents();//graphical representaion
        Thread t = new Thread() {// creat a thread
            public void run() {// implement of thread
                while (true) { //loop
                    DateFormat dateFormat = new SimpleDateFormat("hh:mm:ss aa"); //creat date format 
                    String dateString = dateFormat.format(new Date()).toString(); //setting the datestring 
                    jLabel2.setText(dateString);//reset the date sting
                    try {
                        Thread.sleep(1); //run thread
                    } catch (Exception e) {
    ...
    public void arithmetic_operator(){ //method of arithmatic operatoin
        switch(calculation){ // swith to calculator value
            case 1: //switch the case 1
                ans=num+ Double.parseDouble(jTextField1.getText());  // value added to ans statement updates the text in
